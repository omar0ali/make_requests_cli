...
func GetHTTPRequest(app *tview.Application, pages *tview.Pages, db *gorm.DB) *tview.Flex {
	...
	listRequest.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		case tcell.KeyEnter:
			...
			//Call request.
			body, err := activities.HttpRequest(activities.GET, activities.ConstructURL(selectedRequest), nil)
			if err != nil {
	...
	mainFlex.SetBorder(true).SetTitle("GET HTTP REQUEST")
